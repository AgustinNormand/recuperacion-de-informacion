...
def query(model, tokenizer):
	...
	results = model.query(query_terms)
	#principal_menu = ConsoleMenu("Resultados de query", )
	#principal_menu.show()
	#print("Resultados")
...
def posting(model, tokenizer):
	...
	print("Term: {}, Posting: {}".format(normalized_term, model.get_posting(normalized_term)))
...
def mostrar_menu_principal():
	print("\r\n")
	if re_index == 1:
		docnames_ids, vocabulary, inverted_index, documents_vectors, documents_norm = indexer.index(dirpath, empty_words_path)
	else:
		docnames_ids, vocabulary, inverted_index, documents_vectors, documents_norm = load_files()
		print("Files loaded")

	t = Tokenizer(empty_words_path)
	m = model.Model(docnames_ids, vocabulary, inverted_index, documents_vectors, documents_norm)
