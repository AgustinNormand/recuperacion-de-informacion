...
def sort_inverted_index(final_inverted_index):
	for key in final_inverted_index:
		final_inverted_index[key] = sorted(final_inverted_index[key], key=lambda tup: tup[0])
...
def calculate_idf(final_vocabulary, document_count): ## Se podría refactorizar, Calcular antes.
...
def calculate_documents_norm(documents_norm, final_documents_vectors, vocabulary):
	for key in final_documents_vectors:
		documents_norm[key] = calculate_document_norm(final_documents_vectors[key], vocabulary)
...
def process_results(results):
	...
	documents_norm = {}
	for result in results: #results = [[vocab, inverted_index], [vocab, inverted_index]] 1 por cada thread
	...
	sort_inverted_index(final_inverted_index) #Podría hacerse un insert ordenado, refactor
	calculate_idf(final_vocabulary, len(final_documents_vectors))
	calculate_documents_norm(documents_norm, final_documents_vectors, final_vocabulary)
