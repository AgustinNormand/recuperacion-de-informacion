...
class Model:
    ...
    def build_query_vector(self, query_terms):
        ...
        print("Vector de query: {}, Norma de la query: {}".format(
            self.query_vector, self.query_norm))
    ...
    def get_document_scores(self):
        ...
        for term in self.query_vector:
            ...
            for posting in self.inverted_index[term]:
                ...
                try:
                    self.scalar_product_acumulator[doc_id] += query_term_weight * (
                        document_tf * term_idf)
                except:
                    self.scalar_product_acumulator[doc_id] = query_term_weight * (
                        document_tf * term_idf)
        ...
        for doc_id in self.scalar_product_acumulator:
            try:
                score = self.scalar_product_acumulator[doc_id] / (
                    self.query_norm * self.documents_norm[doc_id])
            except:
                score = 0
            if score != 0:
    ...
    def query(self, query_terms):
        ...
        for value in self.result:
            doc_name = list(self.docnames_ids.keys())[
                list(self.docnames_ids.values()).index(value)]
            browser_path = "file:///home/agustin/Desktop/Recuperacion/repo/TP_02/ejercicio_6/"+doc_name
            final_ordered_result.append(
                (value, self.result[value], browser_path))

        final_ordered_result = sorted(
            final_ordered_result, key=lambda tup: tup[1], reverse=True)

        print("\r\n")
        print(tabulate(final_ordered_result, headers=[
              'DocID', 'Score', 'DocPath'], tablefmt='orgtbl'))
